...
roundingInterval = 3 # Frequency with which TT-rounding is performed
...
if saveTrainedCores:
    ...
    savedCoreName = game+"_ITTD_epsilon"+"".join(format(epsilon,'.2f').split("."))+f"_{trainRunCount}train_{testRunCount}test"
    saveType = "npy"

if printMetrics2File:
    metricsFileName = game+"_ITTD_epsilon"+"".join(format(epsilon,'.2f').split("."))+f"_{trainRunCount}train_{testRunCount}test.txt"
...
while trainRunIndices:
    ...
    iterTime=time.time()
    newShape=tuple(list(dataSet.reshapedShape[:-1])+[-1])
    incrementData=dmt.ttObject(images,method=method,epsilon=epsilon)
    incrementData.changeShape(newShape)
    incrementData.ttDecomp(np.linalg.norm(imagesNorm))
    if (incStep+1)%roundingInterval==0:
        roundTT=True
    else:
        roundTT=False
    dmt.ttObject.ittd(dataSet,incrementData,rounding=roundTT,epsilon=1e-8,)
