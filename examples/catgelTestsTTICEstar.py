...
method = "ttsvd"
epsilon = 0.1
heuristicsToUse = ["skip", "occupancy"]
initialize = 1 # How many simulations we want to increment the TT-cores. Set to 1 in the manuscript.
increment = 1 # How many simulations we want to train the initial set of TT-cores. Set to 1 in the manuscript.
...
if saveTrainedCores:
    ...
    savedCoreName = "Catgel_TTICEStar_"+"_".join(heuristicsToUse)+"_epsilon"+"".join(format(epsilon,'.2f').split("."))+f"_{numberSimulations}train"
    saveType = "npy"

if printMetrics2File:
    metricsFileName = "Catgel_TTICEstar_epsilon"+"".join(format(epsilon,'.2f').split("."))+f"_{numberSimulations}train.txt"
...
if printMetrics2File:
    ...
    lines2print.append(f'{initialize-1}')
