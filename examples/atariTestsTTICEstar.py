...
### Helper function to load image files in parallel
...
occupancyThreshold = 0.8 # Occupancy threshold for occupancy heuristics
...
# List of heuristic upgrades we employ while expanding TT-cores.
# Feel free to explore a subset of heuristics as you wish.
# TODO: Create input flags (maybe?)
...
if saveTrainedCores:
    ...
    savedCoreName = "Catgel_TTICEstar_"+"_".join(heuristicsToUse)+"_epsilon"+"".join(format(epsilon,'.2f').split("."))+f"_{trainRunCount}train_{testRunCount}test"
    saveType = "npy"


if printMetrics2File:
    metricsFileName = game+"_TTICEstar_epsilon"+"".join(format(epsilon,'.2f').split("."))+f"_{trainRunCount}train_{testRunCount}test.txt"
    ...
    lines2print.append(" ".join(heuristicsToUse))
    lines2print.append("\n")
...
while trainRunIndices:
    ...
    dataSet.ttICEstar(
        ...
        elementwiseEpsilon=elementwiseRelErrorBeforeUpdate,
    ...
    if printMetrics2File:
        if "subselect" in heuristicsToUse:
            if previousRanks==dataSet.ttRanks:
                lines2print.append(f'{0}')
                if printMetrics:
                    print(f"Number of images used    : {0}")
            else:
                lines2print.append(f'{(elementwiseRelErrorBeforeUpdate>epsilon).sum()}')
                if printMetrics:
                    print(f"Number of images used    : {(elementwiseRelErrorBeforeUpdate>epsilon).sum()}")
        else:
            lines2print.append(f'{numImgs}')
            if printMetrics:
                    print(f"Number of images used    : {numImgs}")
