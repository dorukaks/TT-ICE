...
if saveTrainedCores:
    ...
    savedCoreName = "Catgel_ITTD_epsilon"+"".join(format(epsilon,'.2f').split("."))+f"_{numberSimulations}train"
    saveType = "npy"

if printMetrics2File:
    metricsFileName = "Catgel_ITTD_epsilon"+"".join(format(epsilon,'.2f').split("."))+f"_{numberSimulations}train.txt"
    ...
    lines2print.append(f"ITTD {roundingInterval}")
    lines2print.append("\n")
...
for snapshotIdx in range(initialize,numberSimulations,increment):
    ...
    iterTime=time.time()
    incrementData=dmt.ttObject(snapshots,epsilon=epsilon,method=method)
    incrementData.ttDecomp(np.linalg.norm(snapshotsNorm))
