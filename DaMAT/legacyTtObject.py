class ttObjectLegacy:
    def __init__(
        ...
        if (
        ...
        ):  # if you are starting with original data (tensor form) you will give numpy arrays as input at the class initialization
            self.nCores = None
            self.ttRanks = ranks
            self.ttCores = None
            ...
            if transposeMap is None:
                self.transposeMap = [idx for idx in range(len(data.shape))]
            else:
                self.transposeMap = transposeMap
        elif (
            self.inputType == list
        ):  # if you are starting with compressed data (tt form) you will give list of arrays as input at the class initialization
            ...
            self.originalShape = [cores.shape[1] for cores in data]
            self.reshapedShape = self.originalShape
            self.imageCount = np.prod(self.originalShape) // (
                np.prod(self.originalShape[:3])
            )
            # self.imageWidth=84
            # self.imageHeight=84
            # self.imageDepth=3
    ...
    def changeShape(self, newShape: tuple):
        ...
        self.singleReshapedImage = self.reshapedShape[
            :-1
        ]  # This line assumes we keep the last index as the samples index and don't interfere with its shape
    ...
    def createTransposeMap(self):
        ...
        rankingIndices = [
            index
            for element, index in sorted(zip(currentIndices, range(len(currentIndices))))
        ]
        magIndices = np.array(
            [
                index
                for element, index in sorted(
                    zip(rankingIndices, range(len(rankingIndices)))
                )
            ],
            dtype=int,
        )
        posIndices = np.arange(0, len(currentIndices), 1, dtype=int)
        combIndices = np.concatenate(
            (posIndices.reshape(-1, 1), magIndices.reshape(-1, 1)), axis=1
        )
    ...
    def indexMap(
        ...
        if baseDimension is None:
            ...
            initialShape = list(initialShape)
            del (
                desiredShape[: len(baseDimension)],
                initialShape[:fixDimensions],
            )  # this line assumes that the original matrix has width*height*RGB as FIRST 3 dimensions
        ...
        if len(initialShape) > len(desiredShape):
            ...
            warnings.warn(
                "Input and output have the same size, returning the same index..."
            )
    ...
    def primeReshaping(self, ordered=False, full: bool = False):
        ...
        if full:
            ...
            for dimensions in originalImageShape:
                self.singleReshapedImage.extend(primes(dimensions))
            factorization = []
            for dimension in self.originalData.shape:
                factorization.extend(primes(dimension))
    ...
    def coresOfImage(self, imageIndex):
        desiredImage = ttObject.indexMap(
            self.originalShape,
            self.reshapedShape,
            imageIndex,
            self.singleReshapedImage,
    ...
    def returnImage(self, imageCores):
        ...
        for index in indices2Remove:
            transposeMap.remove(index)
    ...
    def returnOriginalImage(
        self, imageIndex
    ):  # Replace the functionality of this code appropriately
        ...
        desiredImage = list(np.array(desiredImage)[list(self.transposeMap)])
        index = ""
        for idx in desiredImage:
            index += idx + ","
        index = index[:-1]
        imageArray = eval(f"self.originalData[{index}]")
        transposeMap = list(self.transposeMap)
        indices2Remove = list(
            np.arange(len(imageArray.shape), len(self.originalData.shape), 1, int)
        )
        for index in indices2Remove:
            transposeMap.remove(index)
        imageArray = np.transpose(imageArray, axes=tuple(np.argsort(transposeMap)))
    ...
    def incrementalUpdate(
        ...
        tenNorm=None,
        useHeuristics=False,
        heuristicsToUse="none",
        occupancyThreshold=0.8,
    ):
        if tenNorm is None:
            tenNorm = np.linalg.norm(newTensor)
        newTensor = newTensor.reshape(tuple(list(self.reshapedShape[:-1]) + [-1]))[
            None, :
        ]
        ...
        for coreIdx in range(len(self.ttCores)):
            ...
            if coreIdx == 0:
                ...
                newTensor = (
                    ...
                    .reshape(np.prod(self.ttCores[coreIdx - 1].shape[:-1]), -1)
                    .T
                    @ newTensor
                )
                newTensor = newTensor.reshape(Ui.shape[0], -1)
            if useHeuristics and (
                heuristicsToUse == "all" or heuristicsToUse == "occupancy"
            ):
                if (coreIdx != len(self.ttCores) - 1) and self.coreOccupancy[
                    coreIdx
                ] >= occupancyThreshold:
                    continue
            if coreIdx == len(self.ttCores) - 1:
                if useHeuristics and (
                    heuristicsToUse == "all" or heuristicsToUse == "subselect"
                ):
                    try:
                ...
                newTensor = (
                    self.ttCores[coreIdx - 1]
                    .reshape(-1, self.ttCores[coreIdx - 1].shape[-1])
                    .T
                    @ newTensor
                )
                self.ttCores[coreIdx] = np.concatenate((tempCore, newTensor), axis=1)[
                    :, :, None
                ]
                break
            # elif coreIdx==0:
            ...
            if coreIdx != len(self.ttCores) - 2:
                ...
                tempCore = np.concatenate(
                    (tempCore, np.zeros((URi.shape[1], tempCore.shape[1]))), axis=0
                )
    ...
    def incrementalUpdate2(
        ...
        for coreIdx, core in enumerate(self.ttCores):
            if useHeuristics and (
                ...
                if (coreIdx == len(self.ttCores) - 1) or self.coreOccupancy[
                    coreIdx
                ] > 0.8:
                    if coreIdx != len(self.ttCores) - 1:
                        continue
            if coreIdx == len(self.ttCores) - 1:
                if useHeuristics and (
                    ...
                    self.ttCores[coreIdx] = tempCore[:, :, None]
                    break  # If data is subselected break at update of the last core and update the last core outside with projection!!
                newTensor = self.ttCores[coreIdx - 1].reshape(
                    -1, self.ttCores[coreIdx - 1].shape[-1]
                ).T @ newTensor.reshape(
                    (newTensor.shape[0] * self.ttCores[coreIdx - 1].shape[1], -1)
                )
                self.ttCores[coreIdx] = np.concatenate((tempCore, newTensor), axis=1)[
                    :, :, None
                ]
                ...
                Ri = (
                    np.eye(np.prod(core.shape[:-1]))
                    - core.reshape(np.prod(core.shape[:-1]), -1)
                    @ core.reshape(np.prod(core.shape[:-1]), -1).T
                ) @ newTensor.reshape((np.prod(newTensor.shape[:2]), -1))
            else:
                newTensor = self.ttCores[coreIdx - 1].reshape(
                    -1, core.shape[0]
                ).T @ newTensor.reshape(
                ...
                Ri = (
                    ...
                    - core.reshape(np.prod(core.shape[:-1]), -1)
                    @ core.reshape(np.prod(core.shape[:-1]), -1).T
                ) @ newTensor.reshape(np.prod(self.ttCores[coreIdx].shape[:-1]), -1)
            URi, _, _ = deltaSVD(Ri, tenNorm, newTensorSize, epsilon)
            self.ttCores[coreIdx] = np.hstack(
                (core.reshape(np.prod(core.shape[:-1]), -1), URi)
            ).reshape(self.ttCores[coreIdx].shape[0], self.reshapedShape[coreIdx], -1)
            if coreIdx != len(self.ttCores) - 2:
                Uip1 = self.ttCores[coreIdx + 1].reshape(
                ...
                Uip1 = np.concatenate(
                    (
                        ...
                        np.zeros(
                            (
                                (
                                    self.ttCores[coreIdx].shape[-1]
                                    - self.ttCores[coreIdx + 1].shape[0]
                                )
                ...
                self.ttCores[coreIdx + 1] = Uip1.reshape(
                    self.ttCores[coreIdx].shape[-1],
                    self.ttCores[coreIdx + 1].shape[1],
                    -1,
                )
    ...
    def incrementalUpdateOld(
        self, newTensor, epsilon=None, tenNorm=None, heuristicsToUse="all"
    ):
        ...
        if epsilon is None:
            epsilon = self.ttEpsilon
        for coreIdx, core in enumerate(self.ttCores):
            if heuristicsToUse == "all" and (
                coreIdx == len(self.ttCores) - 1 or self.coreOccupancy[coreIdx] > 0.8
            ):
                continue
            tempData = self.projectData(newTensor, upTo=coreIdx + 1)
            tempData = self.reconstruct(tempData, upTo=coreIdx + 1).reshape(
                tuple(list(self.originalShape[:-1]) + [-1])
            )
            tempData = newTensor - tempData
            tempData = self.projectData(tempData, upTo=coreIdx).reshape(
                np.prod(self.ttCores[coreIdx].shape[:2]), -1
            )
            u, _, _ = deltaSVD(tempData, tenNorm, newTensorSize, eps=epsilon)
            tempCore = np.concatenate(
                (
                    self.ttCores[coreIdx].reshape(
                        np.prod(self.ttCores[coreIdx].shape[:-1]), -1
                    ),
                    u,
                ),
            ...
            self.ttCores[coreIdx] = tempCore.reshape(
                tuple(list(self.ttCores[coreIdx].shape[:2]) + [-1])
            )
            self.ttRanks[coreIdx + 1] = self.ttCores[coreIdx].shape[-1]
            tempCore = self.ttCores[coreIdx + 1].reshape(
                np.prod(self.ttCores[coreIdx + 1].shape[:-1]), -1
            )
            if coreIdx != len(self.ttCores) - 2:
                v = np.zeros(
                    (
                        u.shape[-1] * self.ttCores[coreIdx + 1].shape[1],
                        self.ttCores[coreIdx + 1].shape[-1],
                    )
                )
            else:
                ...
                self.ttCores[coreIdx + 1] = np.concatenate(
                    ...
                    axis=0,
                ).reshape(-1, self.ttCores[coreIdx + 1].shape[1], 1)
                break
            tempCore = np.concatenate((tempCore, v), axis=0)
            self.ttCores[coreIdx + 1] = tempCore.reshape(
                self.ttCores[coreIdx].shape[-1], self.ttCores[coreIdx + 1].shape[1], -1
            )
        self.ttRanks = [1]
        for core in self.ttCores:
            self.ttRanks.append(core.shape[-1])
    ...
    ):  # computes relative error by reconstructing data
    ...
    ):  # Ensure that tt1 and tt2 are instances of ttObject
        if not isinstance(tt1, ttObject) or not isinstance(tt2, ttObject):
            if isinstance(tt1, list):
                tt1 = ttObject(tt1)
            if isinstance(tt2, list):
